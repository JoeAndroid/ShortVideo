...
public class GalleryItemViewHolder extends RecyclerView.ViewHolder {
    ...
    public void setData(final MediaInfo info) {
        ...
        if (info.thumbnailPath != null
            ...
            Glide.with(thumbImage.getContext()).load(uri).centerCrop().dontAnimate()
                    .thumbnail(0.5f).into(thumbImage);
//            ImageLoader.getInstance().displayImage(uri, thumbImage, UILOptions.LOCAL);
    ...
    public void onBind(MediaInfo info, boolean actived, final int position, final GalleryAdapter adapter) {
        setData(info);
